...
> do result :: Maybe ("activities-heart-intraday" := Value) <- fmap decode (L.readFile "fitbit.json"); print result
...
> do result :: Maybe ("activities-heart-intraday" := ("dataset" := [("time" := String, "value" := Int)])) <- fmap decode (L.readFile "fitbit.json"); print result
...
> do Just (_ := (_ := ds)) :: Maybe ("activities-heart-intraday" := ("dataset" := [("time" := String, "value" := Int)])) <- fmap decode (L.readFile "fitbit.json"); mapM_ (print) ds; print (maximum (map (get #value) ds))
